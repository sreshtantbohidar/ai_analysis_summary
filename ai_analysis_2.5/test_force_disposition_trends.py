...
NON_NUMERIC_FIELDS = ['location_name', 'enemy_formation_name', 'formation_type', 'disposition_status', 'equipment.equipment_key', 'equipment.equipment_type', 'activity_type', 'location_type']
NUMERIC_FIELDS = ['equipment_count', 'troop_capacity']
FIELDS_COMBINATION = [
	['location_name', 'enemy_formation_name'],
	['location_name', 'equipment.equipment_key', 'equipment_count'],
	['location_name', 'equipment.equipment_type', 'equipment_count'],
	['location_name', 'formation_type'],
	['location_name', 'disposition_status'],
	['enemy_formation_name', 'activity_type'],
	['enemy_formation_name', 'equipment.equipment_key'],
	['enemy_formation_name', 'equipment.equipment_type'],
	['enemy_formation_name', 'disposition_status'],
	['location_name', 'enemy_formation_name', 'equipment_count'],
	['location_name', 'enemy_formation_name', 'equipment_count'],
	['location_name', 'enemy_formation_name', 'disposition_status'],
	['location_name', 'formation_type', 'equipment.equipment_type', 'equipment_count'],
	['location_name', 'formation_type', 'equipment.equipment_key', 'equipment_count'],
	['location_name', 'formation_type', 'disposition_status'],
	['enemy_formation_name'],
	['disposition_status'],
	['equipment.equipment_key', 'equipment_count'],
	['equipment.equipment_type', 'equipment_count'],
	['activity_type'],
	['location_type']
]
OTHER_FIELDS = ["Year", "Month"]
RENAME_DICT = {
	'location_name': 'Location of Sitrep',
	'enemy_formation_name': 'Name of enemy Formation',
	'formation_type': 'Type of Army Formation',
	'disposition_status': 'Disposition Status',
	'equipment.equipment_key': 'Equipment Key',
	'equipment.equipment_type': 'Equipment Type',
	'activity_type': 'Activity Type',
	'location_type': 'Location Type',
	'equipment_count': 'Total Equipment Count',
	'troop_capacity': 'Total Troop Capacity'
}
...
def json_to_prompt(json_data, category):
    ...
    for raw_combination in json_data:
        ...
        # Custom prompt patterns based on field combinations
        if 'Location of Sitrep' in combination and 'Name of enemy Formation' in combination:
            prompt = f"You are a defense analyst. The {category} data includes enemy formations reported at different locations. Analyze the monthly trends and summarize key patterns."

        elif 'Location of Sitrep' in combination and 'Equipment Key' in combination:
            prompt = f"You are a defense analyst. The {category} activities involve different types of equipment deployed at various locations. Analyze the equipment usage trends and summarize your insights."

        elif 'Location of Sitrep' in combination and 'Equipment Type' in combination:
            prompt = f"You are a defense analyst. The dataset contains {category} equipment types reported at multiple locations. Please analyze the trends and summarize key insights."

        elif 'Location of Sitrep' in combination and 'Type of Army Formation' in combination:
            prompt = f"You are a defense analyst. The {category} data involves different types of army formations deployed across locations. Analyze the trends and provide your findings."

        elif 'Location of Sitrep' in combination and 'Disposition Status' in combination:
            prompt = f"You are a defense analyst. The {category} data includes disposition status of formations across locations. Please summarize the trends and patterns observed."

        elif 'Name of enemy Formation' in combination and 'Activity Type' in combination:
            prompt = f"You are a defense analyst. The data shows types of activities conducted by enemy formations. Please analyze {category} trends and summarize."

        elif 'Name of enemy Formation' in combination and 'Disposition Status' in combination:
            prompt = f"You are a defense analyst. {category} data includes disposition status of enemy formations. Summarize the key deployment trends."

        elif 'Equipment Type' in combination:
            prompt = f"You are a defense analyst. The {category} data includes various equipment types used across formations. Analyze the equipment-related trends."

        elif 'Activity Type' in combination:
            prompt = f"You are a defense analyst. The {category} data shows different activity types performed by formations. Summarize the patterns and insights."

        elif 'Location Type' in combination:
            prompt = f"You are a defense analyst. The data includes {category} trends categorized by location types. Analyze and summarize the key insights."

        else:
            # Fallback
            prompt = f"You are a defense analyst. The dataset contains monthly {category} trends grouped by: {', '.join(combination)}. Please analyze and summarize your insights."

        # Append the relevant JSON data
...
def force_disposition_main(start_date, query):
	...
	prompts = json_to_prompt(result, "Force Disposition")
	html_fragments = []
...
def force_disposition_stats_main(start_date, query):
	base_records = get_base_records(query)
	base_records = verify_fields(base_records, NON_NUMERIC_FIELDS, NUMERIC_FIELDS)
	base_records = base_records[
		[col for col in ['@timestamp'] + NON_NUMERIC_FIELDS + NUMERIC_FIELDS if col in base_records.columns]
...
if __name__ == '__main__':
	...
	html_report = force_disposition_main(start_date, query)
	with open("force_disposition_report.html", "w", encoding="utf-8") as f:
		f.write(html_report)