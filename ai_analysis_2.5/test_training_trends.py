...
CURRENT_INDEX_NAME = constants.CUREENT_INDEX_NAME  # ✅ Fixed typo reference
...
NON_NUMERIC_FIELDS = ['location_name', 'enemy_formation_name', 'formation_type', 'training_type',
                      'equipment.equipment_name', "equipment.equipment_type"]
NUMERIC_FIELDS = ["troop_capacity", "equipment.count"]
FIELDS_COMBINATION = [
	['location_name', 'enemy_formation_name'],
	['location_name', 'equipment.equipment_name', 'equipment.count'],
	['location_name', 'formation_type'],
	['location_name', 'training_type'],
	['enemy_formation_name', 'equipment.equipment_name', 'equipment.count'],
	['enemy_formation_name', 'training_type'],
	['location_name', 'enemy_formation_name', 'equipment.equipment_name', 'equipment.count'],
	['location_name', 'enemy_formation_name', 'training_type'],
	['location_name', 'formation_type', 'training_type'],
	['enemy_formation_name'],
	['training_type'],
	['equipment.equipment_name'],
	['equipment.equipment_type'],
	['location_name'],
	['location_name', 'training_type', 'equipment.equipment_type', 'equipment.count'],
	['training_type', 'equipment.equipment_type']
]
...
def get_llm_response(query, llm_model, timeout=300, retries=5):
	...
	for attempt in range(retries):
		try:
			r = requests.post(anythingllm_url, headers=headers, json=payload, timeout=timeout)
	...
	# Return blank response instead of raising exception
	logger.error(f"Failed to get response from LLM after {retries} retries")
	return ""
...
def sanitize_for_grouping(val):
    """Convert unhashable types to string for safe grouping"""
...
def get_filters(base_records):
    current_data = {}
    filters = {}
    df = base_records.copy()  # ✅ Work on a copy to avoid mutation

    for combination in FIELDS_COMBINATION:
        used_columns = [col for col in combination if col not in NUMERIC_FIELDS]

        # Patch: Convert unhashable types (e.g., lists) to strings in used_columns
        for col in used_columns:
            if col in df.columns:
                df[col] = df[col].apply(sanitize_for_grouping)

        distinct_rows_x = df.drop_duplicates(subset=used_columns)

        # --------------------------
        distinct_rows = distinct_rows_x.copy()
        distinct_rows = distinct_rows[['@timestamp'] + used_columns if '@timestamp' in distinct_rows_x.columns else used_columns]
...
def json_to_prompt(json_data, category):
	...
	for raw_combination in json_data:
		...
		elif 'Location of Training' in combination and 'Equipment Name' in combination and 'Equipment Count' in combination:
		...
		elif 'Location of Training' in combination and 'Training Type' in combination:
			prompt += f"You are a defense analyst. There are {category} activities happening on specific locations by various types of Training types. I have a list of JSON data which has monthly trends of {category} activities happening on different locations by type of training. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
			prompts.append(prompt)
		...
		elif 'Name of Army Formation' in combination and 'Training Type' in combination:
			prompt += f"You are a defense analyst. There are {category} activities happening with specific Army formation performing specific training type. I have a list of JSON data which has monthly trends of {category} activities happening with formation, equipment names and training types. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
			prompts.append(prompt)
		...
		elif 'Location of Training' in combination and 'Equipment Name' in combination and 'Training Type' in combination:
			prompt += f"You are a defense analyst. There are {category} activities happening with specific location performing specific types of training with particular equipment. I have a list of JSON data which has monthly trends of {category} activities happening in various locations with specific equipments and training. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
			prompts.append(prompt)

		elif 'Location of Training' in combination and 'Type of Army Formation' in combination and 'Training Type' in combination:
			prompt += f"You are a defense analyst. There are {category} activities happening with specific location using some types of formation and training types. I have a list of JSON data which has monthly trends of {category} activities happening in various locations with specific army formation anf training type. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
			prompts.append(prompt)
		...
		elif 'Training Type' in combination:
			prompt += f"You are a defense analyst. There are {category} activities happening with specific types of training and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening with specific training types and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
			prompts.append(prompt)
		...
		elif 'Location of Training' in combination and 'Training Type' in combination and 'Equipment Type' in combination and 'Equipment Count' in combination:
			prompt += f"You are a defense analyst. There are {category} activities happening which are certain training type in a location using specific equipment type and equipment count. I have a list of JSON data which has monthly trends of {category} activities happening which are certain training type in a location using specific equipment type and equipment count. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
			prompts.append(prompt)

		elif 'Training Type' in combination and 'Equipment Type' in combination:
			prompt += f"You are a defense analyst. There are {category} activities happening using specific equipment and doing certain training type. I have a list of JSON data which has monthly trends of {category} activities happening using specific equipment and doing certain training type. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
			prompts.append(prompt)
	return prompts
...
def training_main(start_date, query):
...
def training_stats_main(start_date, query):
	base_records = get_base_records(query)
	base_records = verify_fields(base_records, NON_NUMERIC_FIELDS, NUMERIC_FIELDS)
	base_records = base_records[
		[col for col in ['@timestamp'] + NON_NUMERIC_FIELDS + NUMERIC_FIELDS if col in base_records.columns]]
...
# 	query = {"size": 10000, "query": {"bool": {
# 		"must": [{"exists": {"field": "training_type"}}, {"terms": {"daily_activity_type_id": [383]}},
# 		         {"term": {"form_type.keyword": "training"}}, {"exists": {"field": "location_name"}},
# 		         {"range": {"activity_date": {"gte": "2025-05-21T00:00:00", "lt": "2025-06-02T00:00:00"}}}],
# 		"must_not": [{"term": {"form_status": 5}}], "filter": []}}, "sort": [{"@timestamp": {"order": "desc"}}]}
# 	html_report = training_main('2025-06-02', query)
