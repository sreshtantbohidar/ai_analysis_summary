...
NON_NUMERIC_FIELDS = [
    "location_name", "enemy_formation_name", "formation_type",
    "equipment.equipment_name", "equipment.equipment_type",
    "airfield_type", "infra_type", "infra_stage"
]
...
FIELDS_COMBINATION = [
    ...
    ["enemy_formation_name", "airfield_type"],
    [
        "location_name",
    ...
    ["location_name", "enemy_formation_name", "airfield_type"],
    ...
    ["location_name", "formation_type", "airfield_type"],
    ["enemy_formation_name"],
    ["equipment.equipment_name"],
    ["equipment.equipment_type"],
    ["location_name"],
    ["airfield_type"],
    ["location_name", "airfield_type", "equipment.equipment_type", "equipment.count"],
    ["location_name", "airfield_type", "equipment.equipment_name", "equipment.count"],
    ["airfield_type", "equipment.equipment_type", "equipment.count"],
    ["airfield_type", "equipment.equipment_name", "equipment.count"],
    ["location_name", "infra_type"],
    ["location_name", "infra_stage"],
    ["infra_type"],
    ["infra_stage"],
    ["airfield_type", "infra_type"],
    ["airfield_type", "infra_stage"],
    ["location_name", "infra_type"],
    ["location_name", "infra_stage"],
]
...
RENAME_DICT = {
    "location_name": "Location of AIR Ops",
    ...
    "airfield_type": "Airfield Type",
    "infra_type": "Infrastructure Type",
    "infra_stage": "Infrastructure Stage",
...
def merge_both_current_and_history(records):
    ...
    for record in records:
        ...
        key = tuple(
            (k, record[k]) for k in record if k not in ["Present Data", "Past Data"]
        )
        ...
        if key not in merged:
            merged[key] = {
                **{
                    ...
                    if k not in ["Present Data", "Past Data"]
                },
                "Present Data": {},
                "Past Data": {},
            }
...
def detect_trend(current_df, history_df):
    ...
    col_int = [
        col
        for col in current_df.select_dtypes(include="int").columns
        if col not in time_columns
    ]
        ...
        if not match.empty:
            ...
            for _, hist_row in match.iterrows():
                ...
                if not any(val in temp_result.values() for val in values_to_check):
                    rename_temp_result = {
                        RENAME_DICT.get(k, k): v for k, v in temp_result.items()
                    }
...
def build_dynamic_es_query_with_date(
    filters, timestamp_field, start_date, end_date, match_type="term"
):
    should_clauses = []
    ...
    for item in filters:
        ...
        for field, value in item.items():
            if value is None:
                continue
            must_clauses.append({match_type: {field: str(value).strip()}})
...
def get_historical_records(timestamp_field, start_date, end_date, filters_list):
    query = build_dynamic_elastic_query(
        timestamp_field, start_date, end_date, filters_list
    )

    all_docs = scan(
        client=es, index=CURRENT_INDEX_NAME, query=query, scroll="2m", size=1000
    )
    docs = [doc["_source"] for doc in all_docs]
    df = pd.json_normalize(docs, sep=".")
    return df
...
def get_filters(base_records):
    ...
    for combination in FIELDS_COMBINATION:
        ...
        if "@timestamp" in distinct_rows.columns:
            distinct_rows["@timestamp"] = pd.to_datetime(
                distinct_rows["@timestamp"], errors="coerce"
            )
        ...
        for field in NUMERIC_FIELDS:
            if field in distinct_rows.columns:
                ...
                distinct_rows[field] = (
                    pd.to_numeric(distinct_rows[field], errors="coerce")
                    .fillna(0)
                    .astype(int)
                )
            else:
        ...
        group_by_fields = [
            each for each in used_columns if each in distinct_rows.columns
        ]
        ...
        if not numeric_fields_exists:
            grouped_df = grouped_df.drop(
                columns=[col for col in NUMERIC_FIELDS if col in grouped_df.columns]
            )
        current_data[str(combination)] = grouped_df
        result = distinct_rows_x[used_columns].to_dict(orient="records")
        result = [
            {k: ("" if pd.isna(v) else v) for k, v in row.items()} for row in result
        ]
...
def build_dynamic_es_query(filters, match_type="match"):
    ...
    for item in filters:
        ...
        for field, value in item.items():
            ...
            if value is None:
                continue
            clause = {match_type: {field: str(value).strip()}}
...
def historical_search(timestamp_field, start_date, end_date, filters):
    ...
    for combination, filter_list in tqdm(filters.items()):
        ...
        merged_df = get_historical_records(
            timestamp_field, start_date, end_date, filter_list
        )
        ...
        if not merged_df.empty:
            ...
            for field in NUMERIC_FIELDS:
                if field in merged_df.columns:
                    numeric_fields_exists = True
                    merged_df[field] = (
                        pd.to_numeric(merged_df[field], errors="coerce")
                        .fillna(0)
                        .astype(int)
                    )
                else:
            ...
            group_by_fields = [
                each for each in combination_list if each in merged_df.columns
            ]
...
def json_to_prompt(json_data, category):
    ...
    for raw_combination in json_data:
        ...
        prompt = ''
        if "Location of AIR Ops" in combination and "Name of Army Formation" in combination:
            prompt += f"You are a defense analyst. Analyze the trend of {category} activities at specific locations involving different army formations. Here is the monthly JSON trend data: {json.dumps(json_data[raw_combination])}"
        
        elif "Location of AIR Ops" in combination and "Equipment Name" in combination and "Equipment Count" in combination:
            prompt += f"You are a defense analyst. Analyze the {category} trends by location and the use of specific equipment (with counts). Here is the monthly JSON trend data: {json.dumps(json_data[raw_combination])}"
        
        elif "Location of AIR Ops" in combination and "Type of Army Formation" in combination:
            prompt += f"You are a defense analyst. Examine how various army formation types are active across different locations for {category} activities. Here is the monthly JSON trend data: {json.dumps(json_data[raw_combination])}"
        
        elif "Location of AIR Ops" in combination and "Equipment Type" in combination and "Equipment Count" in combination:
            prompt += f"You are a defense analyst. Study {category} activity distribution across locations based on equipment types and their usage count. Here is the JSON trend data: {json.dumps(json_data[raw_combination])}"
        
        elif "Name of Army Formation" in combination and "Equipment Name" in combination and "Equipment Count" in combination:
            prompt += f"You are a defense analyst. Review how different formations use specific equipment in {category} operations. Here is the monthly JSON trend data: {json.dumps(json_data[raw_combination])}"
        
        elif "Name of Army Formation" in combination and "Equipment Type" in combination and "Equipment Count" in combination:
            prompt += f"You are a defense analyst. Study the types and quantity of equipment used by army formations in {category} activity trends. Here's the JSON data: {json.dumps(json_data[raw_combination])}"
        
        elif "Location of AIR Ops" in combination and "Equipment Name" in combination and "Equipment Type" in combination:
            prompt += f"You are a defense analyst. Assess the {category} activities across locations, involving specific equipment and its types. JSON trend data: {json.dumps(json_data[raw_combination])}"

        elif "Location of AIR Ops" in combination and "Type of Army Formation" in combination and "Equipment Type" in combination:
            prompt += f"You are a defense analyst. Examine {category} activities combining location, army formation types, and equipment types. JSON: {json.dumps(json_data[raw_combination])}"

        elif "Name of Army Formation" in combination:
            prompt += f"You are a defense analyst. Provide insights on the trend of {category} activities based on army formations. Monthly JSON data: {json.dumps(json_data[raw_combination])}"
        
        elif "Equipment Name" in combination:
            prompt += f"You are a defense analyst. Evaluate how {category} activities are linked to specific equipment usage. Monthly trend data: {json.dumps(json_data[raw_combination])}"
        
        elif "Equipment Type" in combination:
            prompt += f"You are a defense analyst. Study the trends of {category} activities with respect to types of equipment involved. Data: {json.dumps(json_data[raw_combination])}"
        
        elif "Location of AIR Ops" in combination:
            prompt += f"You are a defense analyst. Assess how {category} activities vary across different locations. Here’s the JSON trend data: {json.dumps(json_data[raw_combination])}"

        elif "Airfield Type" in combination and "Location of AIR Ops" in combination:
            prompt += f"You are a defense analyst. Examine {category} activity trends by airfield types and their geographic locations. JSON data: {json.dumps(json_data[raw_combination])}"
        
        elif "Airfield Type" in combination and "Equipment Type" in combination and "Equipment Count" in combination:
            prompt += f"You are a defense analyst. Analyze {category} trends based on airfield type, types of equipment used, and count. Trend data: {json.dumps(json_data[raw_combination])}"

        elif "Infrastructure Type" in combination and "Location of AIR Ops" in combination:
            prompt += f"You are a defense analyst. Look at {category} infrastructure activity trends by type and location. JSON trend list: {json.dumps(json_data[raw_combination])}"
        
        elif "Infrastructure Stage" in combination and "Location of AIR Ops" in combination:
            prompt += f"You are a defense analyst. Analyze the development stage of infrastructure involved in {category} across different locations. JSON: {json.dumps(json_data[raw_combination])}"

        elif "Infrastructure Type" in combination:
            prompt += f"You are a defense analyst. Evaluate {category} activity trends based on infrastructure types. Data: {json.dumps(json_data[raw_combination])}"

        elif "Infrastructure Stage" in combination:
            prompt += f"You are a defense analyst. Observe trends in {category} activities with respect to infrastructure development stages. JSON: {json.dumps(json_data[raw_combination])}"

        elif "Airfield Type" in combination:
            prompt += f"You are a defense analyst. Study the {category} trends across different types of airfields. JSON data: {json.dumps(json_data[raw_combination])}"
...
def trends_stats_airfield_main(start_date, query):
    base_records = get_base_records(query)
    base_records = verify_fields(base_records, NON_NUMERIC_FIELDS, NUMERIC_FIELDS)
    base_records = base_records[
        [
...
if __name__ == '__main__':
    ...
    html_report = trends_airfield_main('2023-04-21', query)
    with open("airinspect_report.html", "w", encoding="utf-8") as f:
        f.write(html_report)
