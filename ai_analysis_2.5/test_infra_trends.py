...
NON_NUMERIC_FIELDS = ['location_name', 'enemy_formation_name', 'formation_type', 'training_type', 'infra_type', 'infra_stage']
NUMERIC_FIELDS = []
FIELDS_COMBINATION = [
    ['location_name', 'enemy_formation_name'],
    ['location_name', 'formation_type'],
    ['location_name', 'infra_type'],
    ['enemy_formation_name', 'infra_type'],
    ['location_name','enemy_formation_name', 'infra_type'],
    ['location_name', 'formation_type', 'infra_type'],
    ['enemy_formation_name'],
    ['infra_type'],
    ['infra_stage'],
    ['location_name'],
    ['enemy_formation_name'],
    ['location_name', 'infra_stage'],
    ['location_name', 'formation_type', 'infra_stage'],
    ['location_name', 'enemy_formation_name', 'infra_stage']
]

OTHER_FIELDS = ["Year", "Month"]
RENAME_DICT = {
    'location_name': 'Location of Training',
    'enemy_formation_name': 'Name of Army Formation',
    'formation_type': 'Type of Army Formation',
    'training_type': 'Training Type',
    'infra_type': 'Type of Infrastructure',
    'infra_stage': 'Stage of Infrastructure',
...
#     # Define time columns (e.g., ['year', 'month'])
#     time_columns = OTHER_FIELDS
#
#     # Detect string and integer columns excluding time columns
...
#     # if len(col_int) > 1:
#     #     col_int.remove('Count')
#
#
#     result = []
#
#     # Iterate through current_df rows
#     for _, curr_row in current_df.iterrows():
#         # Build dynamic condition for string columns
...
#         current_data = {
#             f"{curr_row['Month']} {curr_row['Year']}": curr_row['Count']
#         }
#
#         if not match.empty:
#             for _, hist_row in match.iterrows():
#                 history_data = {
#                     f"{hist_row['Month']} {hist_row['Year']}": hist_row['Count']
#                 }
#
#                 temp_result = {
...
#         # else:
#         #     # If no history match, return empty history_data and blank year/month
#         #     result.append({
#         #         **{col: curr_row[col] for col in col_str},
#         #         'year': '',
#         #         'month': '',
...
def llm_analysis(historical_combinations_data, current_combinations_data):
    ...
    for combination in current_combinations_data:
        cat_key = combination
        if str(combination) in historical_combinations_data:
...
def get_historical_records(timestamp_field, start_date, end_date, filters_list):
    # query = build_dynamic_es_query_with_date(
    #     filters=filters_list,
    #     timestamp_field=timestamp_field,
    #     start_date=start_date,
    #     end_date=end_date,
    #     match_type="term"  # Change to "match" if needed
    # )
    # print(query)
...
def get_filters(base_records):
    ...
    for combination in FIELDS_COMBINATION:
        ...
        for field in NUMERIC_FIELDS:
            ...
            # df[field] = pd.to_numeric(df.get(field, 0), errors="coerce").fillna(0).astype(int)
...
def historical_search(timestamp_field, start_date, end_date, filters):
    ...
    for combination, filter_list in tqdm(filters.items()):
        ...
        # print('COMBINATION -->', combination_list, f'--> {len(filter_list)} search filters')
        # print(filter_list)
        combination_list = [timestamp_field] + combination_list
        # linear hits
        # df_list = []
        # for filters_dict in tqdm(filter_list):
        #     hist_records = get_historical_records(timestamp_field, start_date, end_date, filters_dict)
        #     if not hist_records.empty:
        #         df_list.append(hist_records)
        ...
        if not merged_df.empty:
            ...
            # print(f'total {merged_df.shape[0]} rows found')
...
def json_to_prompt(json_data, category):
    ...
    for raw_combination in json_data:
        ...
        prompt = ''
        if 'Location of Training' in combination and 'Name of Army Formation' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening on specific locations by various names of army formations and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening on different locations by name of army formations and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Location of Training' in combination and 'Type of Army Formation' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening on specific locations by various types of army formations and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening on different locations by type of army formations and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Location of Training' in combination and 'Type of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening on specific locations by various types of Infrastructure and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening on different locations by type of Infrastructure and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Name of Army Formation' in combination and 'Type of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening with specific Army formation using specific types of Infrastructure and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening with formation and Infrastucture types and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Location of Training' in combination and 'Name of Army Formation' in combination and 'Type of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening with specific locations with specifc formation name with a infrastructure type and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening with specific locations with specifc formation name with a infrastructure type and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Location of Training' in combination and 'Type of Army Formation' in combination and 'Type of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening with specific locations with specifc formation using a particular infrastructure types and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening with specific locations with specifc formation using a particular infrastructure types and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Name of Army Formation' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening with specific army formation names and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening with specific army formation names and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Type of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening using specific types of infrastructure and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening using specific types of infrastructure and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Stage of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening with specific infrastructure stages and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening with specific infrastructure stages and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Location of Training' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening in different locations and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening in different locations and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Name of Army Formation' in combination and 'Stage of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening with different army formation with specific infrastructure stage and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening with different army formation with specific infrastructure stage and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Location of Training' in combination and 'Stage of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening in different locations using specific infrastructure types and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening in different locations using specific infrastructure types and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Location of Training' in combination and 'Type of Army Formation' in combination and 'Stage of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening in different locations using specific types of army formation along with particular infrastructure stage and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening in different locations using specific types of army formation along with particular infrastructure stage and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)

        elif 'Location of Training' in combination and 'Name of Army Formation' in combination and 'Stage of Infrastructure' in combination:
            prompt += f"You are a defense analyst. There are {category} activities happening in different locations with their army formation names along with particular infrastructure stage and their count of occurrences. I have a list of JSON data which has monthly trends of {category} activities happening in different locations with their army formation names along with particular infrastructure stage and their count of occurrences. Can you analyze the JSON data and perform trend analysis and share the summary. Please find the below list of JSON: {json.dumps(json_data[raw_combination])}"
            prompts.append(prompt)
    return prompts
...
def infra_main(start_date, query):
    ...
    prompts = json_to_prompt(result, "Infra")
    html_fragments = []
...
def infra_stats_main(start_date, query):
    base_records = get_base_records(query)
    base_records = verify_fields(base_records, NON_NUMERIC_FIELDS, NUMERIC_FIELDS)
    base_records = base_records[[col for col in ['@timestamp']+NON_NUMERIC_FIELDS+NUMERIC_FIELDS if col in base_records.columns]]
    current_combinations_data, filters = get_filters(base_records)
...
if __name__ == '__main__':
    ...
    html_report = infra_main('2024-01-01', query)
    with open("infra_report.html", "w", encoding="utf-8") as f:
        f.write(html_report)
