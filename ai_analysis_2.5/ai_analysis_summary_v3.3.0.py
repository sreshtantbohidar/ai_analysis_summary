...
# ⭐ v3.3.0: safety caps for the inline chat path. The CLI chunk_token_threshold
...
def get_llm_response_with_data_context(markdown_content: str, query: str, llm_model: str,
                                       ...
                                       workspace_slug: Optional[str] = None) -> str:
    """
    ⭐ v3.3.0: send data to the LLM through an (ephemeral) AnythingLLM workspace.

    The caller creates a brand-new workspace per request (see
    run_llm_request_with_ephemeral_workspace) and passes its slug; each call
    uploads its document straight into that workspace (addToWorkspaces →
    one-step embed) and queries in RAG mode. Because a fresh workspace is
    created, used and deleted for every request, RAG can never return stale
    documents from other runs — the root cause of the repeated "July 2026"
    titles.
...
# ──────────────────────────────────────────────
# ⭐ v3.3.0: Monthly grouping helpers
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# ⭐ v3.3.0: Per-group LLM processing
# ──────────────────────────────────────────────
def run_llm_request_with_ephemeral_workspace(markdown_content: str, query: str, llm_model: str,
                                             doc_label: str, key: str,
                                             ai_analysis_summary_id: int, month: str) -> str:
    """
    ⭐ v3.3.0: run ONE LLM request against a brand-new ephemeral workspace.

    Per-request workspace reset — every single LLM call gets its own fresh
    AnythingLLM workspace:
      1. create_anythingllm_workspace(...)          → fresh workspace
      2. update_anythingllm_workspace_model(...)    → set the model
      3. get_llm_response_with_data_context(...)    → upload + RAG query (chat fallback)
      4. delete_anythingllm_workspace(...)          → always removed (finally)

    Because the workspace is created, used and deleted per request, RAG can
    never return stale documents from other calls/months/runs. If workspace
    creation fails, get_llm_response_with_data_context falls back to exact
    inline chat (no workspace is left behind either way).
    """
    # Short request tag: strip the leading "key_id_month" prefix from the
    # doc_label (already in the workspace name) and keep only the part suffix.
    _label_prefix = f"{key}_{ai_analysis_summary_id}_{month}"
    _tag = doc_label[len(_label_prefix):].lstrip("_") if doc_label.startswith(_label_prefix) else doc_label
    ws_name = (
        f"ai-summary-{key}-{ai_analysis_summary_id}-{month}-"
        f"{sanitize_label(_tag) or 'single'}-{int(time.time() * 1000)}-{os.getpid()}"
    )
    workspace_slug = create_anythingllm_workspace(ws_name)
    if workspace_slug:
    ...
    try:
        return get_llm_response_with_data_context(
            markdown_content, query, llm_model, doc_label=doc_label, workspace_slug=workspace_slug)
    finally:
        if workspace_slug:
            delete_anythingllm_workspace(workspace_slug)


def split_hits_by_token_budget(hits: list, search_form_type: str, query_text: str,
                               max_tokens: int, key: Optional[str] = None) -> List[list]:
    """
    ⭐ v3.3.0: split a group of records into sub-groups (chunks) whose rendered
    markdown + prompt stays under max_tokens.

    Records are packed greedily in the order they arrive (activity_date desc).
    The prompt text is counted once per chunk and a small allowance covers the
    markdown document header / table scaffolding rendered per chunk. Falls back
    to a full single-hit markdown estimate when the type key is unknown.
    """
    _, query_tokens = count_words_and_tokens(query_text)
    CHUNK_OVERHEAD_TOKENS = 120  # # title / summary stats / table header etc.
    fields = TYPE_MAPPING.get(key, {}).get("fields", []) if key else []

    per_hit_tokens = []
    for hit in hits:
        source = hit.get("_source", {}) if isinstance(hit, dict) else {}
        if fields:
            # Mirror convert_hits_to_markdown's 120-char truncation so the
            # estimate matches what is actually rendered (long descriptions
            # would otherwise be over-counted and chunks undersized).
            row_values = []
            for f in fields:
                v = str(source.get(f, ""))
                if len(v) > 120:
            ...
            md = convert_hits_to_markdown([hit], search_form_type)
            _, tokens = count_words_and_tokens(md)
        per_hit_tokens.append(max(tokens, 1))

    budget = max(max_tokens - query_tokens - CHUNK_OVERHEAD_TOKENS, 50)
    chunks = []
    current_chunk = []
    current_tokens = 0
    for hit, hit_tokens in zip(hits, per_hit_tokens):
        if current_chunk and current_tokens + hit_tokens > budget:
            chunks.append(current_chunk)
            current_chunk = []
            current_tokens = 0
        current_chunk.append(hit)
...
def llm_analyze_hits(hits: list, search_form_type: str, title: str, doc_label: str,
    ...
    are consolidated in a final call. ⭐ v3.3.0: every LLM call in here runs
    against its own ephemeral workspace (per-request reset), falling back to
    inline chat.
    """
    ...
    if total_tokens < effective_threshold:
        print(f"[INFO] {doc_label}: within token limit → single RAG call (fresh workspace per request)")
        return run_llm_request_with_ephemeral_workspace(
            markdown_data, full_query, model_name, doc_label=doc_label,
            key=key, ai_analysis_summary_id=ai_analysis_summary_id, month=month)
    ...
    for idx, chunk in enumerate(tqdm(chunks, desc=f"LLM Chunks ({doc_label})", unit="chunk")):
        ...
        summary = run_llm_request_with_ephemeral_workspace(
            chunk_markdown, base_prompt + extra_prompt, model_name,
            doc_label=f"{doc_label}_chunk{idx}",
            key=key, ai_analysis_summary_id=ai_analysis_summary_id, month=month)
    ...
    return run_llm_request_with_ephemeral_workspace(
        ...
        doc_label=f"{doc_label}_consolidated",
        key=key, ai_analysis_summary_id=ai_analysis_summary_id, month=month)
...
def process_month_group(month: str, month_hits: list, search_form_type: str, key: str,
    ...
    If the month's data fits inside the token safe limit a single LLM request is
    made. Otherwise ⭐ v3.3.0: the month's records are split into token-budget
    chunks (split_hits_by_token_budget) and each chunk is analysed separately.
    Every request — single call or chunk — runs against its own fresh
    AnythingLLM workspace (created → model update → upload → RAG query →
    deleted), so RAG can never see stale documents.
    """
    ...
    # limit must still split into chunks instead of overflowing the model.
    effective_threshold = min(chunk_token_threshold, MAX_INLINE_CHAT_TOKENS)
    ...
    if total_tokens < effective_threshold:
        print(f"[MONTH {month}] Within token limit → single RAG call (fresh workspace per request)")
        return run_llm_request_with_ephemeral_workspace(
            month_markdown, full_query, model_name, doc_label=month_doc_label,
            key=key, ai_analysis_summary_id=ai_analysis_summary_id, month=month)

    # ⭐ v3.3.0: over the threshold → split the month's records into token-budget
    # chunks (records kept whole, order preserved) and analyse each separately.
    print(f"[MONTH {month}] Over token limit (~{total_tokens} tokens) → "
          f"splitting {len(month_hits)} records into token-budget chunks")
    chunks = split_hits_by_token_budget(
        month_hits, search_form_type, full_query, effective_threshold, key=key)
    print(f"[MONTH {month}] Found {len(chunks)} record chunk(s)")

    chunk_parts = []
    for idx, chunk_hits in enumerate(chunks, 1):
        chunk_doc_label = f"{month_doc_label}_chunk{idx}"
        print(f"[MONTH {month}] 🔖 Chunk {idx}/{len(chunks)} → {len(chunk_hits)} record(s)")
        chunk_response = llm_analyze_hits(
            chunk_hits, search_form_type,
            title=f"{search_form_type} — {month_label} — Part {idx}/{len(chunks)}",
            doc_label=chunk_doc_label, base_prompt=base_prompt + month_hint, extra_prompt=extra_prompt,
            model_name=model_name, key=key,
            chunk_token_threshold=effective_threshold, chunk_size=chunk_size,
            ai_analysis_summary_id=ai_analysis_summary_id, month=month,
        )  # llm_analyze_hits applies the MAX_CHUNK_TOKENS cap internally
        if chunk_response:
            chunk_parts.append(
                f'<div class="chunk-block"><h3>&#128204; Part {idx} of {len(chunks)} '
                f'({len(chunk_hits)} records)</h3>'
                f'<div class="chunk-content">{_strip_wrappers(chunk_response)}</div></div>'
            )
...
# ──────────────────────────────────────────────
# ⭐ v3.3.0: ai_analysis_summary_check with Monthly Grouping
# ──────────────────────────────────────────────
...
def call_main_func():
    ...
    if args.ai_summary:
        ai_summ = True
        print("AI summary mode is enabled. (Monthly grouped report, per-request ephemeral workspace)")
    else:
